...
describe("entitlement enforcement: API keys", () => {
	...
	test("blocks creation when free-tier limit is reached", async () => {
		...
		// Create 3 keys (free-tier limit)
		await createApiKeyResolver(
			{ input: { name: "key-1", mode: "byok" } },
			buildContext(user),
		);
		await createApiKeyResolver(
			{ input: { name: "key-2", mode: "byok" } },
			buildContext(user),
		);
		await createApiKeyResolver(
			{ input: { name: "key-3", mode: "byok" } },
			buildContext(user),
		);

		// Fourth key should exceed the free-tier limit of 3
		await expect(
			createApiKeyResolver(
				{ input: { name: "key-4", mode: "byok" } },
				buildContext(user),
			),
...
describe("isWithinLimit utility", () => {
	...
	test("returns false when current count equals limit", () => {
		const result = isWithinLimit(null, "max_api_keys", 3, API_KEY_LIMITS);
...
// ── SSOT enforcement audit ──────────────────────────────────────────
// Verify that fallback limits match the Omni API catalog (SSOT)
// so that enforcement is never silently weaker than advertised

describe("fallback limits match SSOT plan configs", () => {
	// From api-stack/services/api/src/lib/db/catalog/planConfigs.ts → synapse
	const SSOT = {
	...
	test("API key fallback limits match SSOT for all tiers", () => {
		expect(API_KEY_LIMITS.max_api_keys.free).toBe(SSOT.free.max_api_keys);
		expect(API_KEY_LIMITS.max_api_keys.pro).toBe(SSOT.pro.max_api_keys);
		expect(API_KEY_LIMITS.max_api_keys.team).toBe(SSOT.team.max_api_keys);
	});

	test("fallback limits are never more permissive than SSOT", () => {
		for (const tier of ["free", "pro"] as const) {
			const ssotLimit = SSOT[tier].max_api_keys;
			const fallbackLimit = API_KEY_LIMITS.max_api_keys[tier];
			// Fallback should never exceed SSOT (would allow more than advertised)
			expect(fallbackLimit).toBeLessThanOrEqual(ssotLimit);
		}
	});
});
